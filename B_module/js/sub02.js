...
function giveDrag() {
  ...
  $$(".drop .item").forEach((e) => {
    ...
    if ($(`.cart .item:has(img[alt="${e.querySelector("img").alt}"])`)) {
      $(`.cart .item:has(img[alt="${e.querySelector("img").alt}"])`).style.opacity = 0.5
    }
  })
...
function dargAndDrop() {
  ...
  drop.addEventListener("drop", (e) => {
    ...
    if ($(`.non-user-container .product-container .item:has(img[alt="${id}"])`)) {
      $(`.non-user-container .product-container .item:has(img[alt="${id}"])`).style.opacity = 0.5
    }
    ...
    if (drop.querySelector(`img[alt="${id}"]`)) {
      const inputValue = drop.querySelector(`img[alt="${id}"]`).closest(".item").querySelector("input").value
      drop.querySelector(`.item:has(img[alt="${id}"])`).querySelector("input").value = parseInt(inputValue) + 1
    } else {
      try {
    ...
    `
        ...
        newBox.addEventListener('dragstart', (event) => {
          const dropId = event.dataTransfer.setData("dropId", newBox.querySelector("img").alt)
          const classCheck = event.dataTransfer.setData("class", newBox.classList)
        ...
        $("body").addEventListener("drop", (event) => {
          ...
          if (classCheck.includes("cloned")) {
            ...
            $(`.non-user-container .product-container .item:has(img[alt="${id}"])`).style.opacity = 1
            priceChange()
          }
        })
...
function priceChange() {
  ...
  let total = 0
  cartItems.forEach((e) => {
    const price = parseInt(e.querySelector(".price").textContent.replace(/[^0-9]/g, ""))
